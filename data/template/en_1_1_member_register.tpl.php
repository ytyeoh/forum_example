...
0
|| checktplrefresh('./template/default/member/register.htm', './template/default/common/seccheck.htm', 1529238625, '1', './data/template/en_1_1_member_register.tpl.php', './template/default', 'member/register')
...
放弃激活，现在<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>">注册帐号</a>
<?php } else { ?>
...
<!--vot-->		<?php if($_GET['action'] != 'activation') { ?>注册<?php } else { ?>您的帐号需要激活<?php } ?>
</h3>
</div>
...
<?php if($bbrules) { ?>
<!--vot-->								<input type="checkbox" class="pc" name="agreebbrule" value="<?php echo $bbrulehash;?>" id="agreebbrule" checked="checked" /> <label for="agreebbrule">!agree_with! <a href="javascript:;" onclick="showBBRule()">网站服务条款</a></label>
<?php } ?>
</span>
</td>
