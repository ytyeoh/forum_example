...
$server_url = 'action=plugins&operation=config&do=' . $pluginid . '&identifier=csdn123com_kuaibao&pmod=keyword';
if ($_GET['formhash'] == FORMHASH && empty($_GET['keyword_add']) == false && $_GET['keyword_add'] == 'yes') {
	...
	$keywordSql="REPLACE INTO " . DB::table('csdn123kuaibao_words') . "(word_str,orderby_num)  VALUES('" . $keyword . "'," . $orderby_num . ")";
	DB::query($keywordSql);
	...
	DB::delete('csdn123kuaibao_words','ID>0');
	cpmsg('csdn123com_kuaibao:succeed', $server_url, 'succeed');
	...
	if (strlen($htmlcode) < 100) {
		cpmsg("csdn123com_kuaibao:hot_keyword_error","","error");
	}
	$remoteUrl = array();
	...
	foreach($htmlcode as $keyword)
	{
		...
		$keywordSql="REPLACE INTO " . DB::table('csdn123kuaibao_words') . "(word_str)  VALUES('" . $keyword . "')";
		DB::query($keywordSql);
	}
	cpmsg('csdn123com_kuaibao:succeed', $server_url, 'succeed');		
	...
	DB::delete('csdn123kuaibao_words','ID=' . $id);
	cpmsg('csdn123com_kuaibao:succeed', $server_url, 'succeed');
	...
	foreach($_GET['ids'] as $id)
	{
		...
		DB::update('csdn123kuaibao_words',$keywordArr,'ID=' . $id);		
	}
	cpmsg('csdn123com_kuaibao:succeed', $server_url, 'succeed');
	
} else {
	
	$keyword_list = DB::fetch_all('SELECT * FROM ' . DB::table('csdn123kuaibao_words') . ' ORDER BY orderby_num ASC,ID DESC');
	include template('csdn123com_kuaibao:keyword');
