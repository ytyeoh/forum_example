...
$csdn123_dingshi = $_G['cache']['plugin']['csdn123com_kuaibao']['csdn123_dingshi'];
$hzw_strict_dingshi = $_G['cache']['plugin']['csdn123com_kuaibao']['hzw_strict_dingshi'];
if ($csdn123_dingshi != 1) {
	echo '// ' . lang('plugin/csdn123com_kuaibao', 'cron_timing_acquisition');
	exit;
}
...
if (DISCUZ_VERSION!='X2.5' && is_numeric($csdn123_cronid) && $csdn123_cronid > 0 && $hzw_strict_dingshi==1) {
	...
	if (is_numeric($csdn123_croninfo['nextrun']) && $csdn123_croninfo['nextrun'] > 0 && $csdn123_croninfo['nextrun'] > TIMESTAMP) {
		...
		echo '// ' . lang('plugin/csdn123com_kuaibao', 'cron_remaining_seconds') . $RemainingSeconds;
		exit;
	}
}
require './source/plugin/csdn123com_kuaibao/function_common.php';
$csdn123_cron = DB::fetch_first("SELECT * FROM " . DB::table('csdn123kuaibao_cron') . " ORDER BY catchtime ASC LIMIT 1");
if (is_numeric($csdn123_cron['catchtime'])) {
	$csdn123_diffTime = time() - $csdn123_cron['catchtime'];
	if ($csdn123_diffTime < 600) {
		echo '// ' .  lang('plugin/csdn123com_kuaibao', 'cron_interval_seconds');
		exit;
	}
}
$csdn123com_kuaibao_chk = DB::fetch_first("SELECT send_datetime FROM " . DB::table('csdn123kuaibao_news') . " ORDER BY send_datetime DESC LIMIT 1");
if (is_numeric($csdn123com_kuaibao_chk['send_datetime'])) {
	$csdn123_diffTime2 = time() - $csdn123com_kuaibao_chk['send_datetime'];
	if ($csdn123_diffTime2 < 600) {
		echo '// ' .  lang('plugin/csdn123com_kuaibao', 'cron_interval_seconds');
		exit;
	}
}
$csdn123_firt_news = DB::fetch_first("SELECT * FROM " . DB::table('csdn123kuaibao_news') . " WHERE tid_aid=0 AND del=0 ORDER BY ID DESC LIMIT 1");
if (empty($csdn123_cron) == false && empty($csdn123_firt_news) == true) {
	...
	if (strlen($htmlcode) < 200) {
		echo '// ' .  lang('plugin/csdn123com_kuaibao', 'collection_result_null');
		exit;
	}
	...
	echo '// ' .  lang('plugin/csdn123com_kuaibao', 'cron_collection_keyword');
} elseif (empty($csdn123_firt_news) == false) {
	require_once './source/function/function_forum.php';
	$status_code = send_thread($csdn123_firt_news['ID']);
	if ($status_code == 'ok') {
		DB::update('csdn123kuaibao_news', array('catch_way' => 'autocatch'), array('ID' => $csdn123_firt_news['ID']));
	}
	$tid_aid_rs = DB::fetch_first("SELECT tid_aid FROM " . DB::table('csdn123kuaibao_news') . " WHERE ID=" . $csdn123_firt_news['ID']);
	echo '// ' . lang('plugin/csdn123com_kuaibao', 'collection_success') . preview_url($csdn123_firt_news['forum_portal'], $tid_aid_rs['tid_aid']);
}
?>